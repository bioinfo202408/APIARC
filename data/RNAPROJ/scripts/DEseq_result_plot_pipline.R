...
option_list <- list(
  ...
  make_option(c("--outdir_table"), type="character"),
  make_option(c("--outdir_plot"), type="character")
)
...
bed_path <- "reference/annotation/gencode.vM25.annotation.bed"
...
num_samples <- nrow(result)
bar_width <- 0.6
spacing_factor <- 0.2
calculated_width <- max(9, num_samples * (bar_width + spacing_factor))
calculated_height <- 5

pdf(file.path(opt$outdir_plot, "hisat2_result_triple_bar.pdf"), 
...
p <- ggplot(result, aes(x = sample, y = percent, fill = type)) +
  ...
  geom_text(aes(label = sprintf("%.2f%%", percent)),
            position = position_dodge(width = bar_width), vjust = -0.3, size = 5) +
  scale_fill_manual(values = c("Unique" = "#4daf4a", "Multiple" = "#377eb8", "Unmapped" = "#e41a1c")) +
  ylim(0, 100) +
  labs(title = "Hisat2 Mapping Statistics", y = "Percentage (%)", fill = "Type") +
  theme_bw() +
...
pca_df$group <- rep(c("Veh", "IP"), each = 2)
...
# Rscript DEseq_result_plot_pipline.R --log /home/yxiaobo/RCProj/RNAseq/database/NMuMG/GSE140552/hisat2_result.log --deg /home/yxiaobo/RCProj/RNAseq/database/NMuMG/GSE140552/mRNA/NMuMG_TGFb12h_vs_veh.csv   --outdir_table /home/yxiaobo/RCProj/RNAseq/database/NMuMG/GSE140552/mRNA --outdir_plot /home/yxiaobo/RCProj/picture/RNAseq/NMuMG