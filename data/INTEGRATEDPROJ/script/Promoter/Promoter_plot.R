...
if (is.null(yaml_info$groups)) q()

chip_csv_files <- file.path(ChIPseq_gene_file, yaml_info$groups, "promoter_peaks.csv")
...
if (length(mean_matrix_list) == 0) q()
...
get_group_means <- function(df, group_name, input_pattern = "Input") {
  ...
  if (length(ip_cols) == 0) q()
  if (length(input_cols) == 0) q()
...
if (length(P_group_name) != 1) q()
if (length(T_group_name) != 1) q()

P_ip_col <- paste0(P_group_name, "_IP_mean")
T_ip_col <- paste0(T_group_name, "_IP_mean")
if (!P_ip_col %in% colnames(Result)) q()
if (!T_ip_col %in% colnames(Result)) q()
