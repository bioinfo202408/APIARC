...
my ($motifdir, $outputdir, $tfmapfile, $motifmapfile, $geneoutput, $groupfile, $bwfile, $outpic, $help);
...
exit 1 unless $motifdir && $outputdir && $groupfile && $bwfile;
...
my %file_samples = %{ $group_config->{file} };
...
foreach my $type (@types) {
    my $posfile_pattern = "$outputdir/*_${type}_peaks.bed";
    ...
    unless (-e $motif_result_dir) {
        mkpath($motif_result_dir, 0755) or exit 1;
    }
    foreach my $posfile (@posfiles) {
        my $prefix = "Promoter_both_${type}_genes";
        ...
        my $networkfile = "$outputdir/TF_motif_network_${type}.txt";
        build_motif_network_with_significance($motif_result_dir, $networkfile);
        if ($tfmapfile && $motifmapfile) {
            my $geneout = $geneoutput // "$outputdir/TF_gene_mapping_${type}.txt";
...
sub extract_fasta_by_genome {
    ...
    my %file_samples = %{ $group_config->{file} };
    my %genome_files = (
        'mm' => "reference/genome/GRCm38.p6.genome.fa.gz",
        'hs' => "reference/genome/GRCh38.p12.genome.fa.gz",
    );
    my $sample = (keys %file_samples)[0];
    my $genome = $file_samples{$sample}->{genome};
    exit 1 unless exists $genome_files{$genome};
    my $genomefile = $genome_files{$genome};
    open(GENOME, "gzip -dc $genomefile|") or exit 1;
...
sub build_motif_network_with_significance {
    my ($outputdir, $outfile) = @_;
    my $motif_result_dir = "$outputdir/Motif_result";
    open(OUT, ">$outfile") or exit 1;
    ...
    foreach my $motifseqfile (@motifseqfiles){
        ...
        open(my $AMETSV, "<", $amefile) or exit 1;
        ...
        close $AMETSV;
        open(MS, "<$motifseqfile") or exit 1;
        while(<MS>){
            chomp;
...
sub build_tf_gene_mapping {
    ...
    open(my $MAP, "<", $tfmapfile) or exit 1;
    while (<$MAP>) {
        chomp;
    ...
    close $MAP;
    my %motifmapHash;
    open(my $MOTIF, "<", $motifmapfile) or exit 1;
    while (<$MOTIF>) {
        chomp;
        my @fieldValues = split /\t/;
        if ($fieldValues[1] =~ /\::/) {
    ...
    my %genetfHash;
    open(my $TE, "<", $tfeRNAfile) or exit 1;
    while (<$TE>) {
        chomp;
        my @fieldValues = split /\t/;
    ...
    close $TE;
    open(my $OUT, ">", $outputfile) or exit 1;
